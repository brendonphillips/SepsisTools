...
#' matching the string given (no extension necessary). If a file is found, it is 
#' read according to its extension; if multiple files matching the string are 
#' found, a warning is given and the first matching file found is read and 
#' returned. If no matching file is found, and empty tibble is returned.
#'
#' @param file_name a string to be matched exactly (with or without file extension) 
...
#' 
#' @return a tibble with the data read from the file matching the string expression given
#' 
#' @examples None
#' 
#' @export
get_table <- function(file_name, ..., FOLDER_ = NA) {
    ...
    return(
        {if(extension == "csv") {
            ...
            paste(
                "'.%s' is not a recognised data file extension. Either change the file",
                "request '%s', or add the extension '.%s' to the get_table function in",
                "'maternal_sibling_stool_header.R' file in the '%s' folder",
                sep=" "
            ) %>%
