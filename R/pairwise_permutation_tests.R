...
#'
#' @description
#' Conducts all possible pairwise permutation tests between the groups in the data set given and a given reference group. 
#' 
#' * If requested, the Global permutation test is performed over the entire data set, and reported in the result data frame of this function. 
#' 
#' * If family correction method(s) are given, the base R p.adjust routine is called to adjust the p.value calculated from the pairwise permutation tests.
#'  
#' * Progress bars (from txtProgressBar) are shown in all instances, but a `verbose` option (TRUE/FALSE) controls output to the terminal after each test. 
#' 
#' * Given that this function calls the global_permutation_test function on the backend (just with different input tables), the only other arguments needed (vs the global call) are the `compare_to`, 'step_down_procedures` and `global_test_first`.
#'
...
#' @param event_col_name the name fo the column with the event (data type castable to numerical)
...
#' @param compare_to for each pairwise test, the group that will be used for comparison
#' @param p_adj_meths a list of family corrections to be performed on the series of p-values obtained from the pairwise tests. Since this is a call to the base p.adjust method, the list of corrections can be found by running `?p.adjust.methods`. To summarise here:
#' * "bonferroni"
#'
#' * "holm" - Holm method,
#'
...
#'
#' @param global_test_first TRUE/FALSE whether a global permutation tests should be performed on the data set before the full set of pairwise tests
#' @param verbose TRUE/FALSE whether summary output from each (global/pairwise) test should be shown in the terminal
#' 
#' @return A tibble (data frame) with the columns:
#' 
#'  * `group_name` the non-reference group in the pairwise test (for a global test, "__GlobaL__"),
#'
#'  * `N_obs` the total number of observations used in each permutation tests (that is, from all included groups),
#'
#'  * `N_trials` the number of permutations done
#'
#'  * `p` the pe value obtained after performing the corresponding test
#'
#'  * `error` the Monte Carlo error term calculated for the test
#'
#'  * `compare_to` the group to be included in all pairwise tests (i.e., a placebo group in a trial)
#'
#'  * `p_adj_...` the adjusted p value of the test, by each requested method in `p_adj_meths`. The adjusted values form each test are given in different columns, with the name "p_adj_" followed by the name of the method (from p.adjust.methods)
#'
